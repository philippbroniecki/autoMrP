...
###########################################################################
# Lme4 convergence tests of random effects specification ------------------
###########################################################################

run_ranef_test <- function(y, L1.x, L2.x, pc_names, L2.unit, L2.reg, loss.unit,
                           loss.fun, data, verbose, cores){
  ...
  if (is.null(L2.reg)) {
    ...
    L2_re <- paste(paste("(1 | ", L2.reg, "/", L2.unit, ")", sep = ""),
                   collapse = " + ")
    L2_unit <- paste("(1 | ", L2.unit, ")", sep = "")
  }

  # Individual-level random effects
  L1_x <- paste("(1 | ", L1.x, ")", sep = "")

  # random effects
  re <- c(L1_x, L2_re)

  # Models
  ranef_list <- lapply(seq_along(re), function(x) {combn(re, x)})
  ranef_list <- lapply(ranef_list, function(x) {
    apply(x, 2, function(ff) {
      formula(paste(y, " ~ ", paste(ff, collapse = " + "), sep = ""))
    })
  }) %>%
    unlist()
  ...
  # without region
  re <- c(L1_x, L2_unit)
  ranef_list2 <- lapply(seq_along(re), function(x) {combn(re, x)})
  ranef_list2 <- lapply(ranef_list2, function(x) {
    apply(x, 2, function(ff) {
      formula(paste(y, " ~ ", paste(ff, collapse = " + "), sep = ""))
    })
  }) %>%
  ...
  # Combine models in tibble
  ranef_models <- dplyr::tibble(
    model = c(ranef_list, ranef_list2),
    region = c(rep(1, length(ranef_list)), rep(0, length(ranef_list2)))
  )

  # Convergence warnings for each model
  m_errors <- lapply(seq(from = 1, to = nrow(ranef_models)) , function(m) {

    # Print model m
    if (isTRUE(verbose)) {
      M <- dim(ranef_models)[1]
      cat(paste("Lme4 tests for random effects specification: Running model ", m,
                " out of ", M, " models\n", sep = ""))
    }
    ...
    k_errors <- lapply(seq_along(data), function(k) {
      ...
      model_m <- best_subset_classifier(model = dplyr::pull(.data = ranef_models, var = model)[[m]],
                                        data.train = data_train,
                                        model.family = binomial(link = "probit"),
                                        model.optimizer = "bobyqa",
                                        n.iter = 1000000,
                                        verbose = verbose)
      singluar <- lme4::isSingular(model_m,tol =  1e-04)
    })
    any(unlist(k_errors))
  })
  ...
  # update formula
  models <- lapply(ranef_list, function(x){
    update(re[[1]], paste("~ . +", x))
  })

  if (!is.null(pc_names)){
    # add PCs
    pc_list <- lapply(seq_along(pc_names), function(x) {combn(pc_names, x)})
    ...
    # update formula
    pc_models <- lapply(pc_list, function(x){
      update(re[[1]], paste("~ . +", x))})
    pc_models <- c(re, pc_models)
  } else{
    pc_models <- NA
  }
